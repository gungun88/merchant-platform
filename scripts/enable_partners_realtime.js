...
async function enablePartnersRealtime() {
  try {
    console.log('🔍 检查partners表的Realtime配置...\n')

    // 执行SQL命令启用Realtime
    const { data, error } = await supabase.rpc('exec_sql', {
      sql: `
        -- 将partners表添加到realtime publication
    ...
    if (error) {
      ...
      console.log('--------------------------------------------------')
      console.log('ALTER PUBLICATION supabase_realtime ADD TABLE partners;')
      console.log('ALTER TABLE partners REPLICA IDENTITY FULL;')
      console.log('--------------------------------------------------\n')
      console.log('或者在 Supabase Dashboard:')
      console.log('1. 进入 Database → Replication')
      console.log('2. 找到 partners 表')
      console.log('3. 启用 "Enable Realtime" 开关\n')
      return
    }

    console.log('✅ Partners表的Realtime功能已启用!')
